...
final class Toc {
    ...
    private final ArrayList<Short> instrs = new ArrayList<Short>();
    private final ArrayList<Statement> stmts = new ArrayList<Statement>();
    private final ArrayList<ArrayList<String>> comments = new ArrayList<ArrayList<String>>();
    private final ArrayList<Link> links = new ArrayList<Link>();
    private final HashMap<String, Label> allLabels = new HashMap<String, Label>();
    ...
    private void ensureCommentsArraySize() {
        ...
        final int m = pc + 1;
        // note: +1 because comments before the statement are added to next pc
        for (int i = n; i < m; i++) {
            comments.add(null);
        }
    }
    ...
    public void addLabel(Token tk, String name, boolean isFunc) throws Throwable {
        ...
        allLabels.put(getScopePrefix() + name, newLbl);
        // e.g. 'print.done' where 'done' is the label
        if (isFunc) {
            enterFunc(tk, name);
        } else {
            if (!scopes.isEmpty()) {
                // if within the scope of a function add the label to current scope
    ...
    public void link() throws Throwable {
        for (Link lnk : links) {
            ...
            switch (lnk.type) {
            case CALL: {
                // get function from global scope using 'name'
                Label lbl = allLabels.get(lnk.name);
                if (lbl == null)
                    throw new Exception(lnk.token.sourcePos() + ": function '" + lnk.name + "' not found");
                if (!lbl.isFunc)
                    throw new Exception(lnk.token.sourcePos() + ": label '" + lbl.name + "' is not a function");
                assert ((lbl.pc & 0xf) == 0);
                instr |= lbl.pc;
                break;
            }
            case JMP: {
                // get label from current scope
                Label lbl = allLabels.get(lnk.scopedName);
                if (lbl == null) {
            ...
            case LDI: {
                // get from global scope using 'name'
                Label lbl = allLabels.get(lnk.name);
                if (lbl == null) {
                    throw new Exception(lnk.token.sourcePos() + ": function '" + lnk.name + "' not found");
                }
    ...
    private void appendComments(StringBuilder sb, int i) {
        ...
        for (String s : cmnts) {
            if (s == null || s.length() == 0) {
                // ? find out where an empty comment is inserted
                continue;
            }
